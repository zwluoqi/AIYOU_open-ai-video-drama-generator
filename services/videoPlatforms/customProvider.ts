...
export class CustomVideoPlatformProvider implements VideoPlatformProvider {
    ...
    private buildPromptContent(
        ...
        if (params.config.aspect_ratio) {
            configParts.push(`比例: ${params.config.aspect_ratio}`);
        }
        if (params.config.duration) {
            configParts.push(`时长: ${params.config.duration}秒`);
        }
        if (params.config.quality) {
            configParts.push(`质量: ${params.config.quality}`);
        }

        if (configParts.length > 0) {
            content += `\n\n视频参数: ${configParts.join(', ')}`;
        }
