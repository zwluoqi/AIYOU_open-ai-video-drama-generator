        ...
        try {
            ...
            if (node.type === NodeType.STORYBOARD_VIDEO_GENERATOR) {
                ...
                if (promptOverride === 'generate-prompt') {
                    ...
                    const visualStyle = getVisualPromptPrefix(style, genre, setting);

                    console.log('[STORYBOARD_VIDEO_GENERATOR] Calling AI with', selectedShots.length, 'shots, style:', style);

                    // Generate prompt using Generic format (no black screen for storyboard videos)
                    const generatedPrompt = await builder.build(selectedShots, { visualStyle });
                ...
                if (promptOverride === 'generate-video') {
                    ...
                    try {
                        ...
                        if (node.data.fusedImage) {
                            // 自定义平台使用 chat/completions 多模态格式，支持 base64，无需上传 OSS
                            if (selectedPlatform === 'custom') {
                                console.log('[STORYBOARD_VIDEO_GENERATOR] 自定义平台：直接使用 base64 图片，跳过 OSS 上传');
                        ...
                        if (!apiKey) {
                            ...
                            const configPath = selectedPlatform === 'custom'
                                ? '设置 → API 配置 → 视频平台 API Keys → 自定义视频平台'
                                : `设置 → API 配置 → 视频平台 API Keys → ${platformName} Key`;
                ...
                for (const name of charactersNeedingGeneration) {
                    ...
                    if (config.method === 'LIBRARY' && config.libraryId) {
                        ...
                        try {
                            ...
                            const generatedProfile = await handleCharacterActionNew(
                                id,                  // nodeId
                            ...
                            // 直接用返回的 profile 更新 newGeneratedChars，避免从过时的 nodesRef 读取
                            if (generatedProfile) {
                                const idx = newGeneratedChars.findIndex(c => c.name === name);
                                if (idx >= 0) {
                                    newGeneratedChars[idx] = { ...generatedProfile };
                                }
                                console.log('[CHARACTER_NODE] Updated newGeneratedChars from returned profile for:', name);
                            }
                    ...
                    // 每个角色处理完后，用最新的 newGeneratedChars 更新节点
                    handleNodeUpdate(id, { generatedCharacters: [...newGeneratedChars] });
                }
